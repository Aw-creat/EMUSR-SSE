...
import aesED
from aesED import decrypt_by_index
...
def get_file(Index, key):
    df = pd.read_csv("doc/encdata/data_index.csv")
...
def get_file_1(Index):
    decrypted_rows = decrypt_by_index("doc/encdata/data_index_per_row.csv", Index)
    with open('./doc/search_file.csv', 'w', encoding='utf-8', newline='') as f:
        csv_writer = csv.writer(f)
...
def update_delete_1(Enindex_tree, trapdoor,enc_path):
    delete_tree,file = Update.delete(Enindex_tree, trapdoor)
    df = pd.read_csv("doc/encdata/data_index_per_row.csv")
    for i in file:
        df = df.drop(df.index[df['id'] == i])
    data = np.array(df.loc[:, :])  # 按行读取文件
    with open(enc_path, 'w', encoding='utf-8', newline='') as f:
